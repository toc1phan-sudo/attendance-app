...
export default function AttendPage() {
  ...
  useEffect(() => {
    ...
    // Nếu URL có ?session=XYZ thì tự điền mã
  ...
  const submit = async () => {
    if (!code || !name || !mssv || submitting) return
    setSubmitting(true)
    ...
    try {
      ...
      if (data.ok) {
        ...
        setErrorMsg(data.reason || 'Có lỗi xảy ra')
        setStep('error')
      }
    } catch {
      setErrorMsg('Không kết nối được server. Kiểm tra mạng và thử lại.')
      setStep('error')
    }
    setSubmitting(false)
  }
  ...
  const s = { // styles
    ...
    btn:     (disabled) => ({
      ...
      background: disabled ? '#ccc' : '#111', color: '#fff',
      ...
      <div style={{ background: '#0ea5e9', padding: '1rem', marginBottom: '1rem' }}>
        <div style={s.wrap}>
          ...
          <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 12, marginTop: 2 }}>
            Nhập thông tin để xác nhận có mặt
          </div>
        </div>
      </div>
      ...
      <div style={s.wrap}>
        <div style={s.card}>
          ...
          {step === 'form' && (
            <>
              <div style={s.group}>
                <label style={s.label}>Mã QR (nhìn trên màn hình chiếu)</label>
                <input
                  value={code}
                  onChange={e => setCode(e.target.value.toUpperCase())}
                  placeholder="VD: ABC-7X2K-9QR"
              ...
              {gpsStatus === 'denied' && (
                <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 10, padding: '12px 14px', marginBottom: 14 }}>
                  ...
                  <div style={{ fontSize: 12, color: '#9a3412', lineHeight: 1.6, marginBottom: 6 }}>
                    Tên bạn sẽ được ghi chú trong danh sách.<br/>
                    Để bỏ ghi chú: bật lại vị trí trong cài đặt trình duyệt rồi tải lại trang.
                  </div>
                  <div style={{ fontSize: 11, color: '#c2410c', fontStyle: 'italic' }}>
                    Chrome: nhấn 🔒 trên thanh địa chỉ → Vị trí → Cho phép<br/>
              ...
              <button
                onClick={submit}
                disabled={!code || !name || !mssv || submitting}
                style={s.btn(!code || !name || !mssv || submitting)}
              >
                {submitting
              ...
              <p style={{ fontSize: 11, color: '#aaa', textAlign: 'center', marginTop: 10 }}>
                Mã QR thay đổi mỗi 60 giây. Không chia sẻ cho người khác.
              </p>
            </>
          )}
            ...
            <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              ...
              {result?.flags?.length > 0 && (
                <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 10, padding: '10px 14px', fontSize: 12, color: '#9a3412', marginBottom: 16, textAlign: 'left' }}>
                  ⚠️ Lưu ý: có một số thông tin cần giáo viên xác nhận thêm.
                </div>
              )}
